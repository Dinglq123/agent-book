...
coverImage: "imgs/01-concept-param-cover-01.png"
capturedAt: "2026-07-26T05:24:51.987Z"
---
...
| 参数 | 类型 | 默认值 | 描述 |
| --- | --- | --- | --- |
| [do\_sample](https://docs.bigmodel.cn/cn/guide/start/concept-param#do_sample) | 布尔值 | `true` | 是否对输出进行采样，以增加多样性。 |
| [temperature](https://docs.bigmodel.cn/cn/guide/start/concept-param#temperature) | 浮点数 | (依赖模型) | 控制输出的随机性，值越高越随机。 |
| [top\_p](https://docs.bigmodel.cn/cn/guide/start/concept-param#top_p) | 浮点数 | (依赖模型) | 通过核采样控制多样性，建议与 `temperature` 二选一。 |
| [max\_tokens](https://docs.bigmodel.cn/cn/guide/start/concept-param#max_tokens) | 整数 | (依赖模型) | 限制单次调用生成的最大 token 数。 |
| [stream](https://docs.bigmodel.cn/cn/guide/start/concept-param#stream) | 布尔值 | `false` | 是否以流式方式返回响应。 |
| [thinking](https://docs.bigmodel.cn/cn/guide/start/concept-param#thinking) | 对象 | `{"type": "enabled"}` | 是否开启思维链深度思考，仅 `GLM-4.5` 及以上支持。 |
| [reasoning\_effort](https://docs.bigmodel.cn/cn/guide/start/concept-param#reasoning_effort) | 字符串 | `max` `xhigh` `high` `medium` `low` `minimal` `none` | 控制模型的推理程度，仅 `GLM-5.2` 及以上支持。 |
