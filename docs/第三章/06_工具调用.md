...
coverImage: "imgs/06-function-calling-cover-01.png"
capturedAt: "2026-07-26T05:26:16.337Z"
---

# 工具调用

大模型本身擅长理解语言、分析问题和生成内容，但它并不天然拥有外部系统的实时数据，也不能仅靠文字直接完成“查天气”“订机票”“查询订单”“写入数据库”这类具体操作。如果只让模型凭记忆回答，它可能会给出过时信息；如果只让模型生成一段说明，它也无法真正替用户完成任务。

工具调用（Function Calling）就是让模型连接外部能力的一座桥。开发者可以先定义一组可调用的函数或 API，告诉模型每个工具能做什么、需要哪些参数；当用户提出需求时，模型会判断是否需要调用工具，并生成符合规范的调用参数。对入门者来说，可以把它理解为：模型负责理解意图和决定步骤，工具负责执行真实操作或获取真实数据。

这项能力之所以重要，是因为它把大模型从“会聊天的助手”扩展成“能办事的智能体”。很多实际应用都离不开工具调用，例如客服系统查询订单、办公助手创建日程、数据分析助手读取数据库、智能体分步骤完成任务。掌握工具调用，就等于掌握了让模型接入业务系统、实时信息和自动化流程的核心方法。
